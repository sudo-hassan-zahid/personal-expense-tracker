    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
      <section className="xl:col-span-4 bg-(--color-surface-card-dark) rounded-xl border border-(--color-hairline-on-dark) p-5">
        ...
        <ActionForm action={saveBudget} successMessage="Budget saved" className="grid gap-3">
          ...
          <HelpLabel
            help="The expense category this monthly budget watches."
          ...
          <HelpLabel
            help="The maximum planned spend for this category before rollover."
            className="mb-[-0.25rem]"
          >
            Monthly limit
          </HelpLabel>
          ...
          />
          <HelpLabel
            help="Extra unspent money carried into this month, if you track rollover manually."
            className="mb-[-0.25rem]"
          >
            Rollover amount
          </HelpLabel>
          ...
          />
          <HelpLabel
            help="The percent of the available budget that turns the budget card into an alert state."
            className="mb-[-0.25rem]"
          >
            Alert %
          </HelpLabel>
          <input
            ...
            className="form-control"
          />
          <SubmitButton className="bg-(--color-primary) text-(--color-on-primary) rounded-lg py-2 text-button">
            Save Budget
          </SubmitButton>
        </ActionForm>
        <div className="mt-5 grid gap-2">
          {budgets.length === 0 ? (
            <p className="text-body-sm text-(--color-muted)">No budgets for this month.</p>
          ) : (
            budgets.map((budget) => (
              <div
                key={budget.id}
                className="flex items-center justify-between gap-3 border border-(--color-hairline-on-dark) rounded-lg p-3"
              >
                <div>
                  <div className="text-body-sm text-(--color-on-dark)">{budget.category}</div>
                  <div className="text-caption text-(--color-muted)">
                    {formatCurrency(
                      Number(budget.limit_amount) + Number(budget.rollover_amount || 0),
                      currency
                    )}{" "}
                    available
                  </div>
      ...
      <section className="xl:col-span-4 bg-(--color-surface-card-dark) rounded-xl border border-(--color-hairline-on-dark) p-5">
        ...
        <ActionForm action={saveSavingsGoal} successMessage="Goal saved" className="grid gap-3">
          <HelpLabel
            help="A short name for the target, like Emergency fund or Laptop."
            className="mb-[-0.25rem]"
          >
            Goal name
          </HelpLabel>
        ...
        <div className="mt-5 grid gap-2">
          {goals.length === 0 ? (
            <p className="text-body-sm text-(--color-muted)">No savings goals yet.</p>
          ) : (
            goals.map((goal) => {
              const pct = Math.min(
                100,
              ...
              return (
                <div
                  key={goal.id}
                  className="border border-(--color-hairline-on-dark) rounded-lg p-3"
                >
      ...
      <section className="xl:col-span-4 bg-(--color-surface-card-dark) rounded-xl border border-(--color-hairline-on-dark) p-5">
        <div className="flex items-center justify-between gap-2 mb-4">
          <div className="flex items-center gap-2">
            ...
            <HelpTip label="Recurring transactions help">
              Save repeating income or expenses, then post due items when they need to become real
              transactions.
            </HelpTip>
          </div>
          <button
        ...
        <ActionForm
          action={saveRecurringTransaction}
          successMessage="Recurring item saved"
          className="grid gap-3"
        >
          <HelpLabel
            help="Whether the repeating item adds money or spends money."
            className="mb-[-0.25rem]"
          >
            Type
          </HelpLabel>
          ...
          <HelpLabel
            help="The amount to post each time this recurring item runs."
            className="mb-[-0.25rem]"
          >
            Amount
          </HelpLabel>
          ...
          />
          <HelpLabel
            help="The category or income source used when this recurring item is posted."
            className="mb-[-0.25rem]"
          >
            Category or source
          </HelpLabel>
          ...
          <HelpLabel
            help="How often this item should repeat after the next date."
            className="mb-[-0.25rem]"
          >
            Frequency
          </HelpLabel>
          ...
          />
          <HelpLabel
            help="Optional text copied onto each posted transaction."
            className="mb-[-0.25rem]"
          >
            Note
          </HelpLabel>
          ...
          {recurring.map((item) => (
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-caption text-(--color-muted)">
                  {item.frequency} from {item.next_date}
                </div>
              </div>
              <button
                onClick={() =>
